...
- What it does:
  ...
  - Executes `test/npm-shared-library-core.test.js`, which scans the `test/` folder for `*.test.ts` files and runs them using `ts-node` via `npx`.

Notes:

- `test/npm-shared-library-core.test.js` will attempt to run tests with `npx ts-node --transpile-only <file>` so you don't need to add `ts-node` as a permanent devDependency.
...
- `test/unit/` — unit tests that do NOT require a WinCC OA installation. These are executed by the
  `test/npm-shared-library-core.test.js` bootstrap and are suitable for running on GitHub-hosted runners.
