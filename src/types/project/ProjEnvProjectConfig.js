...
export class ProjEnvProjectConfig {
     ...
     */
    insertValue(value, key, _section = 'general') {
        ...
        try {
            ...
            if (!sections[_section])
                sections[_section] = Object.create(null);
            // Insert only if key does not already exist
            if (Object.prototype.hasOwnProperty.call(sections[_section], key))
                return -1;
     ...
     */
    setValue(value, key, _section = 'general') {
        const cfgPath = this.getConfigPath();
        if (!cfgPath)
            return -1;
        try {
     ...
     */
    deleteValue(value, key, _section = 'general') {
        const cfgPath = this.getConfigPath();
        if (!cfgPath)
            return -1;
        try {
            ...
            if (sections[_section][key] !== String(value ?? ''))
                return -1;
     ...
     */
    deleteEntry(key, _section = 'general') {
        const cfgPath = this.getConfigPath();
        if (!cfgPath)
            return -1;
        try {
    ...
    getEntryValue(key, _section = 'general') {
        const sectionData = this.getSection(_section);
    ...
    saveConfigSections(sections) {
        ...
        if (!sections['general'])
            sections['general'] = Object.create(null);
        ...
        for (const sec of sectionOrder) {
            ...
            const keys = Object.keys(entries).sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
